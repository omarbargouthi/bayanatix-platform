...
import type { BusinessDomain, SourceLite, UserLite, FoiReportData, FoiRequestRow } from "@/lib/queries/reports";
...
function buildColumns(t: I18nStrings): GridColumn<FoiRequestRow>[] {
  const rt = t.reports.foi;
  return [
    { key: "referenceCode", label: rt.colRef },
    { key: "subjectText", label: rt.colSubject },
    { key: "statusCode", label: t.reports.common.colStatus },
    { key: "officerName", label: rt.colOfficer, render: (r) => r.officerName ?? rt.unassigned },
    { key: "firstResponseDueDate", label: rt.colDue, render: (r) => r.firstResponseDueDate ?? "—" },
    { key: "closedAt", label: rt.colClosed, render: (r) => r.closedAt ?? "—" },
  ];
}

function FoiReportContent({
  ...
  const [data, setData] = useState<FoiReportData | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/reports/foi?${params.toString()}`);
  ...
  function setParam(key: "domain" | "source" | "owner", value: string) {
    ...
    params.delete("page");
    router.push(`/reports/foi?${params.toString()}`);
  }
  ...
  async function captureSnapshot() {
    setCapturing(true);
    try {
      await fetch("/api/reports/R6_FOI/snapshot", { method: "POST" });
      await load();
    } finally {
      setCapturing(false);
    }
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <a href={`/api/reports/R6_FOI/export?${exportParams.toString()}`} className="text-sm px-3 py-2 rounded-lg bg-brand-purple text-white hover:bg-brand-violet">{rc.exportXlsx}</a>
          <a href={`/api/reports/R6_FOI/export-pdf?${exportParams.toString()}`} className="text-sm px-3 py-2 rounded-lg border border-brand-purple text-brand-purple hover:bg-brand-purple/5">{rc.exportPdf}</a>
        </div>
      </div>
        ...
        <>
          ...
          <div className="card-padded">
            ...
            <DrillDownGrid
              ...
              rowKey={(r) => r.foiRequestId} linkHref={(r) => `/foi/${r.foiRequestId}`}
...
export function FoiReportClient(props: { domains: BusinessDomain[]; sources: SourceLite[]; owners: UserLite[]; isAdmin: boolean; domainLocked?: boolean }) {
  return (
    <Suspense fallback={<div className="py-20 text-center text-muted">Loading…</div>}>
      <FoiReportContent {...props} />
    </Suspense>
  );
}
