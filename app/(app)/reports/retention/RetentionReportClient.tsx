...
import type { BusinessDomain, SourceLite, UserLite, RetReportData, RetOverdueRow } from "@/lib/queries/reports";
...
function RetentionReportContent({
  ...
  const [data, setData] = useState<RetReportData | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/reports/retention?${params.toString()}`);
  ...
  function setParam(key: "domain" | "source" | "owner", value: string) {
    ...
    params.delete("page");
    router.push(`/reports/retention?${params.toString()}`);
  }
  ...
  async function captureSnapshot() {
    setCapturing(true);
    try {
      await fetch("/api/reports/R9_RETENTION/snapshot", { method: "POST" });
      await load();
    } finally {
      setCapturing(false);
    }
...
export function RetentionReportClient(props: { domains: BusinessDomain[]; sources: SourceLite[]; owners: UserLite[]; isAdmin: boolean; domainLocked?: boolean }) {
  return (
    <Suspense fallback={<div className="py-20 text-center text-muted">Loading…</div>}>
      <RetentionReportContent {...props} />
    </Suspense>
  );
}
