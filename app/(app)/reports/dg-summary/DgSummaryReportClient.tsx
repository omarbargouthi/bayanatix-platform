...
import type { BusinessDomain, SourceLite, UserLite, DgSummaryReportData } from "@/lib/queries/reports";
...
function DgSummaryReportContent({
  ...
  const [data, setData] = useState<DgSummaryReportData | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/reports/dg-summary?${params.toString()}`);
  ...
  function setParam(key: "domain" | "source" | "owner", value: string) {
    ...
    params.delete("page");
    router.push(`/reports/dg-summary?${params.toString()}`);
  }
  ...
  async function captureSnapshot() {
    setCapturing(true);
    try {
      await fetch("/api/reports/R8_DG_SUMMARY/snapshot", { method: "POST" });
      await load();
    } finally {
      setCapturing(false);
    }
            ...
            <div className="card-padded">
              ...
              <div className="flex flex-wrap gap-2">
                {openTasksKpi.breakdown.map((b) => (
                  <span key={b.label} className="text-xs bg-canvas px-2.5 py-1 rounded-full text-ink-soft">
