...
function buildColumns(t: I18nStrings): GridColumn<DsiAgreementRow>[] {
  const rt = t.reports.dsi;
  return [
    { key: "dsaReferenceCode", label: rt.colRef },
    { key: "titleText", label: rt.colTitle },
    { key: "statusCode", label: t.reports.common.colStatus },
    { key: "sharingScopeCode", label: rt.colScope, render: (r) => r.sharingScopeCode ?? "—" },
    { key: "counterpartyName", label: rt.colCounterparty, render: (r) => r.counterpartyName ?? "—" },
    { key: "effectiveEndDate", label: rt.colExpires, render: (r) => r.effectiveEndDate ?? "—" },
  ];
}
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <a href={`/api/reports/R4_DSI/export?${exportParams.toString()}`} className="text-sm px-3 py-2 rounded-lg bg-brand-purple text-white hover:bg-brand-violet">{rc.exportXlsx}</a>
          <a href={`/api/reports/R4_DSI/export-pdf?${exportParams.toString()}`} className="text-sm px-3 py-2 rounded-lg border border-brand-purple text-brand-purple hover:bg-brand-purple/5">{rc.exportPdf}</a>
        </div>
      </div>
