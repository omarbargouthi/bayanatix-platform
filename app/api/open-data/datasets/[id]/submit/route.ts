...
export async function POST(req: Request, { params }: Ctx) {
  ...
  if (action === "submit") {
    ...
    try {
      ...
      const piiWithoutDeident = await sql<{ physicalName: string; piCategoryCode: string | null }[]>`
        SELECT a.physical_name_text AS "physicalName", bg.pi_category_code AS "piCategoryCode"
        FROM bayanat.open_dataset_columns odc
        JOIN bayanat.data_attributes a ON a.attribute_id = odc.attribute_id
        LEFT JOIN bayanat.asset_business_terms abt
          ON abt.asset_type_code = 'DATA_ATTRIBUTES' AND abt.asset_id = odc.attribute_id AND abt.term_role = 'CLASSIFICATION'
      ...
      if (piiWithoutDeident.length > 0) {
        const names = piiWithoutDeident.map((c) => `"${c.physicalName}"${c.piCategoryCode ? ` (${c.piCategoryCode})` : ""}`).join(", ");
        return NextResponse.json({
      ...
      await sql`
        ...
        VALUES (${reqRow.requestId}, 'OPEN_DATASET', ${datasetId}, ${ds.name})
      ...
      console.error("[open-data submit]", err);
