...
export async function POST(_req: Request, { params }: Ctx) {
  ...
  try {
    ...
    if (!dsa.entity_role_code)           failures.push("PDPL entity role is required.");
    ...
    if (dsa.sharing_scope_code !== "INTERNAL" && !dsa.risk_assessment_ref) {
      failures.push("G5: Risk assessment reference is required for external agreements (NDI DSI.MQ.4).");
    }

    // ── Must have at least one dataset ───────────────────────────────────────
    const [dsCount] = await sql`
      SELECT COUNT(*)::int AS cnt FROM bayanat.dsa_datasets WHERE dsa_id = ${dsaId}
    `;
    if (Number(dsCount.cnt) === 0) {
      failures.push("No datasets added. Add at least one dataset with attributes before submitting.");
    }
    ...
    const [unclassified] = await sql`
      ...
      LEFT JOIN bayanat.asset_business_terms abt
        ...
        AND abt.asset_id = da.attribute_id
    ...
    // ── G4: PI indicator set on every classified attribute's term ─────────────
    // Checks via business_glossaries.is_pii_indicator (no is_pii column on data_attributes)
    const [noPiTag] = await sql`
      SELECT COUNT(*)::int AS cnt
      FROM bayanat.dsa_attributes da
      JOIN bayanat.dsa_datasets dd ON dd.dsa_dataset_id = da.dsa_dataset_id
      ...
      WHERE dd.dsa_id = ${dsaId}
        AND bg.is_pii_indicator IS NULL
    `;
    if (Number(noPiTag.cnt) > 0) {
      failures.push(`G4: ${noPiTag.cnt} attribute(s) have no PI/non-PI indicator on their classification term. Update the glossary term to set the PI flag.`);
    }
    ...
    const [computed] = await sql`
      SELECT
        ...
        EXISTS (
          ...
          JOIN bayanat.business_glossaries bg ON bg.glossary_id = abt.glossary_id
          WHERE dd.dsa_id = ${dsaId}
            AND COALESCE(bg.is_pii_indicator, false) = true
        ) AS has_pii
    `;
    ...
    // ── Snapshot classification codes + PI flags on dsa_attributes ────────────
    // Using a subquery form because PostgreSQL UPDATE...FROM cannot join on
    // the updated table's alias in the FROM clause.
    await sql`
      UPDATE bayanat.dsa_attributes
      SET
        classification_code_snapshot = sub.classification_code,
        is_personal_data_indicator   = COALESCE(sub.is_pii_indicator, false)
      FROM (
        SELECT
          da.dsa_attribute_id,
          bg.classification_code,
          bg.is_pii_indicator
        FROM bayanat.dsa_attributes da
        JOIN bayanat.dsa_datasets dd ON dd.dsa_dataset_id = da.dsa_dataset_id
        LEFT JOIN bayanat.asset_business_terms abt
      ...
      WHERE bayanat.dsa_attributes.dsa_attribute_id = sub.dsa_attribute_id
    `;
    ...
    // Delete any previous approvals, insert fresh set
    ...
    console.error("[DSA submit]", err);
    const message = err instanceof Error ? err.message : String(err);
    return NextResponse.json({ ok: false, failures: [`Server error: ${message}`] }, { status: 500 });
  }
}
