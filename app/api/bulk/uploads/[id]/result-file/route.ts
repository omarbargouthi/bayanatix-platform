...
import { getBulkJob, getBulkJobResultFile } from "@/lib/queries/bulk-jobs";
...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  if (!job || (session.role !== "ADMIN" && job.createdByUserId !== session.userId)) {
    return NextResponse.json({ error: "Result not available yet (upload hasn't been committed)" }, { status: 404 });
  }
