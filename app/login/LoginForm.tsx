"use client";

import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";

type ProviderType = "LOCAL" | "LDAP" | "OIDC";

const PROVIDER_LABEL: Record<ProviderType, string> = {
  LOCAL: "Email & Password",
  LDAP: "LDAP / Active Directory",
  OIDC: "Single Sign-On",
};

export default function LoginForm({ redirectTo, initialError }: { redirectTo: string; initialError?: string }) {
  const router = useRouter();
  const [enabled, setEnabled] = useState<ProviderType[] | null>(null);
  const [active, setActive] = useState<ProviderType | null>(null);
  const [email, setEmail] = useState("khaled@bayanatix.demo");
  const [password, setPassword] = useState("Bayanatix123!");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(initialError ?? null);

  useEffect(() => {
    fetch("/api/auth/config")
      .then((r) => r.json())
      .then((d: { local: boolean; ldap: boolean; oidc: boolean }) => {
        const methods: ProviderType[] = [
          ...(d.local ? (["LOCAL"] as const) : []),
          ...(d.ldap ? (["LDAP"] as const) : []),
          ...(d.oidc ? (["OIDC"] as const) : []),
        ];
        setEnabled(methods);
        setActive(methods[0] ?? null);
      })
      .catch(() => { setEnabled(["LOCAL"]); setActive("LOCAL"); });
  }, []);

  function selectProvider(p: ProviderType) {
    setActive(p);
    setError(null);
    // The bundled demo credentials only make sense against LOCAL auth.
    if (p === "LOCAL") {
      setEmail("khaled@bayanatix.demo");
      setPassword("Bayanatix123!");
    } else {
      setEmail("");
      setPassword("");
    }
  }

  async function onSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!active || active === "OIDC") return;
    setError(null);
    setLoading(true);
    try {
      const r = await fetch("/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ provider: active, email, password }),
      });
      if (!r.ok) {
        const j = await r.json().catch(() => ({}));
        setError(j.error || "Sign-in failed");
        return;
      }
      router.replace(redirectTo);
      router.refresh();
    } catch {
      setError("Network error. Please try again.");
    } finally {
      setLoading(false);
    }
  }

  if (enabled === null || active === null) {
    return <div className="h-40" />; // avoid a form flash before we know which methods to render
  }

  if (enabled.length === 0) {
    return (
      <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-md px-3 py-2">
        No sign-in method is currently enabled. Contact your administrator.
      </div>
    );
  }

  return (
    <div className="space-y-4">
      {enabled.length > 1 && (
        <div className="grid gap-1.5 p-1 bg-canvas-soft rounded-lg" style={{ gridTemplateColumns: `repeat(${enabled.length}, 1fr)` }}>
          {enabled.map((p) => (
            <button
              key={p}
              type="button"
              onClick={() => selectProvider(p)}
              className={`text-[12px] font-semibold px-2 py-1.5 rounded-md transition-colors ${
                active === p ? "bg-white text-brand-deep shadow-sm" : "text-ink-soft hover:text-ink"
              }`}
            >
              {PROVIDER_LABEL[p]}
            </button>
          ))}
        </div>
      )}

      {error && (
        <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-md px-3 py-2">
          {error}
        </div>
      )}

      {active === "OIDC" ? (
        <a
          href={`/api/auth/oidc/login?redirectTo=${encodeURIComponent(redirectTo)}`}
          className="btn btn-primary w-full justify-center !py-2.5"
        >
          Sign in with SSO
        </a>
      ) : (
        <form onSubmit={onSubmit} className="space-y-4">
          <div>
            <label className="field-label" htmlFor="email">{active === "LDAP" ? "Username or email" : "Work email"}</label>
            <input
              id="email"
              type={active === "LDAP" ? "text" : "email"}
              autoComplete="username"
              required
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="field-input"
              placeholder="you@organization.gov.sa"
            />
          </div>

          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="field-label !mb-0" htmlFor="password">Password</label>
              {active === "LOCAL" && <a className="text-xs text-brand-purple font-medium hover:underline" href="#">Forgot?</a>}
            </div>
            <input
              id="password"
              type="password"
              autoComplete="current-password"
              required
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              className="field-input"
              placeholder="••••••••"
            />
          </div>

          <button type="submit" className="btn btn-primary w-full justify-center !py-2.5" disabled={loading}>
            {loading ? "Signing in…" : "Sign in"}
          </button>

          {active === "LOCAL" && (
            <div className="rounded-md border border-dashed border-line bg-canvas-soft px-3 py-2.5 text-[11px] text-ink-soft">
              <strong className="text-ink">Demo credentials</strong> · email
              <code className="font-mono mx-1">khaled@bayanatix.demo</code> · password
              <code className="font-mono ml-1">Bayanatix123!</code>
            </div>
          )}
        </form>
      )}
    </div>
  );
}
