...
/** Bulk-reject: discards every PENDING/DUPLICATE row in the checked set. */
export async function bulkDiscardDqSuggestions(suggestionIds: number[], userId: string): Promise<number[]> {
  const rows = await sql<{ suggestionId: number; status: string }[]>`
    SELECT suggestion_id AS "suggestionId", status_code AS status FROM bayanat.dq_rule_suggestions WHERE suggestion_id = ANY(${suggestionIds})
  `;
  const discarded: number[] = [];
  for (const r of rows) {
    if (r.status !== "PENDING" && r.status !== "DUPLICATE") continue;
    await discardDqRuleSuggestion(r.suggestionId, userId);
