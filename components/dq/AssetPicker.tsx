...
export type SelectedAsset = {
  ...
  assetId: number;
  assetName: string;
  /** Full path: "SourceName / SchemaName / TableName" or "…/ ColumnName" */
  path: string;
  /** Breadcrumb segments for display */
  breadcrumb: string[];
};
...
type Props = {
  ...
  /** If set, the picker starts at this entity and shows columns directly */
...
function PkBadge() {
  return <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-brand-purple/10 text-brand-purple border border-brand-purple/20">PK</span>;
}

function ColPane<T>({
  title, items, activeId, onSelect, loading, renderItem,
}: {
  title: string;
  items: T[];
  ...
  return (
    <div className="flex flex-col border-r border-line last:border-r-0 w-[200px] shrink-0">
      <div className="px-3 py-2 bg-canvas-soft border-b border-line text-[10px] uppercase tracking-wider font-bold text-muted">{title}</div>
        ...
        {!loading && items.map((item) => {
          ...
          return (
            <div
              ...
              className={`cursor-pointer px-3 py-2 border-b border-line-soft last:border-b-0 hover:bg-canvas transition-colors ${active ? "bg-brand-purple/8 border-l-2 border-l-brand-purple" : ""}`}
            >
...
export function AssetPicker({ selected, onChange, preFilterEntityId, preFilterEntityName, onClose }: Props) {
  ...
  // path context for building breadcrumbs when pre-filtered
  ...
  useEffect(() => {
    if (preFilterEntityId) {
      // pre-filter mode: skip source/schema panes, go straight to columns
      setLoadingSrc(false);
      loadColumnsForEntity(preFilterEntityId);
      // resolve source/schema names for breadcrumbs
  ...
  function loadColumnsForEntity(entityId: number) {
    setLoadingCol(true);
  ...
  function selectTable(table: BrowseTable) {
  ...
  function isTableSelected(table: BrowseTable): boolean {
    return selected.some((s) => s.assetType === "DATA_ENTITIES" && s.assetId === table.id);
  }

  function isColumnSelected(col: BrowseColumn): boolean {
    return selected.some((s) => s.assetType === "DATA_ATTRIBUTES" && s.assetId === col.id);
  }

  function buildBreadcrumb(extra: string[]): string[] {
    if (preFilterEntityId) {
      return [
        preFilterContext?.sourceName ?? "…",
        preFilterContext?.schemaName ?? "…",
        preFilterEntityName ?? "…",
        ...extra,
      ];
    }
    return [
      ...
      if (i === 0) return !!activeSource;
      if (i === 1) return !!activeSchema;
      if (i === 2) return !!activeTable || extra.length > 0;
      return true;
    });
  }
  ...
  function toggleTable(table: BrowseTable) {
    if (isTableSelected(table)) {
      ...
      const crumb = [activeSource?.name ?? "—", activeSchema?.name ?? "—", table.name].filter(Boolean);
      onChange([...selected, {
  ...
  function toggleColumn(col: BrowseColumn) {
    if (isColumnSelected(col)) {
      ...
      const crumb = [sourceName, schemaName, tableName, col.name].filter(Boolean);
      onChange([...selected, {
  ...
  const showColumnSelectAll = columns.length > 0 && (activeTable != null || preFilterEntityId != null);

  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-2xl border border-line flex flex-col w-[860px] max-h-[82vh]">

        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-line shrink-0">
          <div>
            <h2 className="font-bold text-ink">Select Assets</h2>
            <p className="text-[11px] text-muted mt-0.5">Navigate Source → Schema → Table → Column and check assets to include in this rule</p>
          </div>
          <button onClick={onClose} className="text-muted hover:text-ink text-xl leading-none ml-4">&times;</button>
        </div>

        {/* Column browser */}
        <div className="flex flex-1 overflow-hidden border-b border-line">

          {/* Sources pane — hidden in pre-filter mode */}
          {!preFilterEntityId && (
            <ColPane
              title="Source"
              items={sources}
          ...
          {/* Schemas pane — hidden in pre-filter mode */}
          {!preFilterEntityId && (
            <ColPane
              title="Schema"
              items={schemas}
          ...
          {/* Tables pane — hidden in pre-filter mode */}
          {!preFilterEntityId && (
            <div className="flex flex-col border-r border-line w-[200px] shrink-0">
              ...
              <div className="flex-1 overflow-y-auto nice-scroll">
                ...
                {!loadingTbl && tables.length === 0 && <div className="px-3 py-4 text-xs text-muted">{activeSchema ? "No tables" : "Select a schema"}</div>}
                {!loadingTbl && tables.map((tbl) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-start gap-2.5 px-3 py-2 border-b border-line-soft last:border-b-0 hover:bg-canvas transition-colors cursor-pointer ${active ? "bg-brand-purple/5 border-l-2 border-l-brand-purple" : ""}`}
                    >
                      <input
                        type="checkbox"
                        checked={checked}
                      ...
                      <div className="min-w-0">
                        <div className={`text-[12px] font-semibold truncate ${active ? "text-brand-purple" : "text-ink"}`}>{tbl.name}</div>
                        <div className="text-[10px] text-muted">
                          {tbl.isView ? "View" : "Table"}{tbl.rowCount != null ? ` · ${fmtRows(tbl.rowCount)} rows` : ""}{tbl.colCount > 0 ? ` · ${tbl.colCount} cols` : ""}
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div className="flex-1 overflow-y-auto nice-scroll">
              ...
              {!loadingCol && columns.length === 0 && (
                <div className="px-3 py-4 text-xs text-muted">
                  {(activeTable || preFilterEntityId) ? "No columns" : "Select a table"}
                </div>
              )}
              {!loadingCol && columns.map((col) => {
                const checked = isColumnSelected(col);
                return (
                  <label
                    key={col.id}
        ...
        <div className="px-5 py-3 shrink-0 min-h-[80px] max-h-[160px] overflow-y-auto">
          ...
          {selected.length === 0 && (
            <p className="text-xs text-muted">No assets selected. Check tables or columns in the browser above.</p>
          )}
          <div className="flex flex-wrap gap-2">
            {selected.map((a) => (
              <div
                key={`${a.assetType}-${a.assetId}`}
                className="flex items-center gap-1.5 bg-canvas-soft border border-line rounded-md pl-2.5 pr-1.5 py-1 text-[11px] group"
              >
                ...
                <span className="text-muted hidden sm:inline">{a.breadcrumb.slice(0, -1).join(" › ")} › </span>
                <span className="font-semibold text-ink">{a.breadcrumb[a.breadcrumb.length - 1]}</span>
                <button
                  onClick={() => removeSelected(a)}
                  className="ml-1 text-muted hover:text-red-500 text-sm leading-none"
                  title="Remove"
                >×</button>
              </div>
            ))}
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-5 py-3 border-t border-line bg-canvas-soft shrink-0 rounded-b-xl">
          <span className="text-[11px] text-muted">
            {selected.length === 0
              ? "Select at least one asset"
              : `${selected.length} asset${selected.length !== 1 ? "s" : ""} selected — will create ${selected.length} rule${selected.length !== 1 ? "s" : ""}`}
          </span>
