...
import { IconBell, IconCog, IconSearch, IconLogout, IconMenu } from "./icons";
...
export function Header({ crumbs, user }: { crumbs: Crumb[]; user: SessionUser }) {
  ...
  return (
    <header className="sticky top-0 z-20 h-[60px] bg-white border-b border-line flex items-center gap-3 px-5">
      ...
      {/* Breadcrumb */}
      <nav className="flex items-center gap-1.5 text-sm text-muted shrink-0">
        {crumbs.map((c, i) => {
          ...
          return (
            <span key={i} className="flex items-center gap-1.5">
              ...
              {!last && <span className="text-line-soft select-none">›</span>}
            </span>
          );
        })}
      </nav>
      ...
      {/* Search */}
      <div className="ml-3 flex-1 max-w-lg flex items-center gap-2 bg-canvas border border-line rounded-md px-3 py-2">
      ...
      <div className="ml-auto flex items-center gap-1">
        <span className="hidden sm:inline-flex items-center gap-1 px-2.5 py-1 border border-line rounded-md text-xs font-semibold text-ink-soft select-none">
          EN ▾
        </span>

        <button className="w-8 h-8 grid place-items-center rounded-md text-ink-soft hover:bg-canvas hover:text-brand-purple transition-colors">
          <IconCog className="w-[17px] h-[17px]" />
        </button>

        <button className="relative w-8 h-8 grid place-items-center rounded-md text-ink-soft hover:bg-canvas hover:text-brand-purple transition-colors">
